...
hr_data_all <- mutate(hr_data_all,
                      years_of_service = interval(`Hire Date`,
                                                  YEAR_OF_SERVICE_CALC_DATE) / years(1)) %>%
...
# TODO: is there a repository for wage increase info? These are  currently hard-coded.
...
# Rename the sal column..
hr_data_all <- rename(hr_data_all,
